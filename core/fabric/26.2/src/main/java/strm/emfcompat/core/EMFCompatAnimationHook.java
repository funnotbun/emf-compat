...
public final class EMFCompatAnimationHook extends EMFAnimationApi.EMFAnimationHook {
    ...
    // What vanilla puts on the model while crouching (HumanoidModel#setupAnim): the rotation is
    // absolute, the two heights are added to the part's initial pose. Packs read these back to
    // detect the crouch, so the exact values matter.
    ...
    private static void emfcompat$repairCrouchPose(EMFEntityRenderState state, EMFModelPartRoot root) {
        ...
        body.y = body.getInitialPose().y() + CROUCH_BODY_Y;
        head.y = head.getInitialPose().y() + CROUCH_HEAD_Y;
    }
