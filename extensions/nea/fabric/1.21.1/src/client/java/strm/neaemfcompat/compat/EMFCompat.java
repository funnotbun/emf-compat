...
public class EMFCompat {
    ...
    private static boolean shouldForceVanillaModel(EMFEntity emfEntity) {
        // Read inside the condition rather than at registration, so the config toggle applies
        // immediately instead of only on the next launch. The NeoForge/Forge modules are missing
        // this check — see the note in the port.
